      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="flex-shrink-0">
            <span className={`font-bold text-lg tracking-tight transition-colors duration-500 ${transparent ? 'text-white' : 'text-toss-dark'}`}>OPIc Example</span>
          </Link>
            ...
            {session ? (
              <>
                {session.user?.role === 'ADMIN' && (
                  <Link href="/admin" className={`text-base font-semibold transition-colors duration-500 ${transparent ? 'text-white/70 hover:text-white' : 'text-toss-gray600 hover:text-toss-dark'}`}>
                    관리자
                  </Link>
                )}
                <Link href="/mypage" className={`text-base font-semibold transition-colors duration-500 ${transparent ? 'text-white/70 hover:text-white' : 'text-toss-gray600 hover:text-toss-dark'}`}>
                  마이페이지
                </Link>
                <button
                  onClick={() => signOut({ callbackUrl: '/' })}
                  className={`text-base font-semibold transition-colors duration-500 ${transparent ? 'text-white/55 hover:text-white' : 'text-toss-gray500 hover:text-toss-dark'}`}
                >
                  로그아웃
                </button>
              </>
